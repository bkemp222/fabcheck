...
export function MobileReview({ project, updateProject }: MobileReviewProps) {
  ...
  return (
    <div className="mobile-page-enter space-y-4 p-4 pb-24">
      {conceptImage ? (
        ...
        <section className="rounded-xl border border-black/5 bg-white p-4 text-sm text-zinc-500 shadow-sm">
          Upload a concept image to generate a budget range.
        </section>
      )}

      <section className="rounded-xl bg-black p-5 text-white shadow-sm">
        <p className="text-xs font-black uppercase tracking-[0.25em] text-white/40">
          Estimated Fabrication Budget
        </p>
        <p className="mt-2 text-4xl font-black italic tracking-tight">
          {budgetRange?.label || "Pending"}
        </p>
        <p className="mt-3 text-xs leading-5 text-white/60">
          Ballpark custom fabrication range based on your selected project type,
          footprint, uploaded concept, and starter notes.
        </p>
      </section>
